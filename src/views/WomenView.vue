...
onMounted(async () => {
    await useStore().getAllProducts()
        .then(() => {
            paginationEnd.value = useStore().products.filter(product => product?.type === "women" ? product : null).length;
            paginationStart.value = useStore().products.filter(product => product?.type === "women" ? product : null).length - 6;
        })
        .then(() => {
